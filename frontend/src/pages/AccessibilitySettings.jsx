...
export default function AccessibilitySettings() {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    // Save notifications
      fetch(`${'https://echoaidbackend.onrender.com'}/api/auth/notifications`, {
      method: 'PUT',
      headers: {
      ...
      fetch(`${'https://echoaidbackend.onrender.com'}/api/auth/privacy`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
function TwoFactorSettings() {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('token');
    if (!token) return;
      fetch(`${'https://echoaidbackend.onrender.com'}/api/auth/me`, {
  ...
  const startSetup = async () => {
    try {
      ...
      setError('');
      const token = localStorage.getItem('token');
      const resp = await fetch(`${'https://echoaidbackend.onrender.com'}/api/auth/2fa/setup`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmEnable = async () => {
    try {
      ...
      setError('');
      const token = localStorage.getItem('token');
      const resp = await fetch(`${'https://echoaidbackend.onrender.com'}/api/auth/2fa/enable`, {
        method: 'POST',
  ...
  const disable = async () => {
    try {
      ...
      setError('');
      const token = localStorage.getItem('token');
      const resp = await fetch(`${'https://echoaidbackend.onrender.com'}/api/auth/2fa/disable`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await resp.json().catch(() => ({}));
