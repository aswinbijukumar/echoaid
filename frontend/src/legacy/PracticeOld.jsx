...
<<<<<<< HEAD:frontend/src/legacy/PracticeOld.jsx
const API_BASE_URL = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api`;
=======
  const API_BASE_URL = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api`;
>>>>>>> dc62a1aeab24bf46cb3b9305bc8d4f9124e3d6d1:frontend/src/pages/PracticeOld.jsx
...
export default function Practice() {
  ...
  const [practiceMode, setPracticeMode] = useState('review'); // 'review' | 'weak' | 'speed' | 'accuracy'
  ...
  const fetchPracticeData = async () => {
    try {
      ...
      // Fetch recent signs (learned in last 7 days)
      const recentResponse = await fetch(`${API_BASE_URL}/api/practice/recent`, {
        headers: {
      ...
      if (recentResponse.ok) {
        ...
        setRecentSigns(recentData.data || []);
      }

      // Fetch weak signs (low accuracy)
      const weakResponse = await fetch(`${API_BASE_URL}/api/practice/weak`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (weakResponse.ok) {
        ...
        setWeakSigns(weakData.data || []);
      }

      // Fetch daily goal
      const goalResponse = await fetch(`${API_BASE_URL}/api/user/progress`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (goalResponse.ok) {
        ...
        setDailyGoal(goalData.data?.dailyGoal || { completed: 0, target: 5 });
      }
      ...
      setRecentSigns([
        { id: 1, word: 'Hello', accuracy: 85, lastPracticed: new Date() },
        { id: 2, word: 'Thank you', accuracy: 92, lastPracticed: new Date() },
        { id: 3, word: 'Goodbye', accuracy: 78, lastPracticed: new Date() }
      ]);
      setWeakSigns([
        { id: 4, word: 'Please', accuracy: 45, lastPracticed: new Date() },
        { id: 5, word: 'Sorry', accuracy: 52, lastPracticed: new Date() }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={`${bg} min-h-screen p-6`}>
      <div className="max-w-7xl xl:max-w-[1400px] mx-auto">
        {/* Notifications */}
        {notification && (
          <div className={`mb-4 p-4 rounded-xl border ${notification.type === 'success'
              ...
              <div className="flex">
                {['webcam', 'upload'].map(tab => (
                  <button
                    ...
                    className={`flex-1 py-3 md:py-4 text-sm md:text-base font-semibold rounded-t-2xl transition-colors ${currentMode === tab
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 lg:gap-3">
                ...
                <div className="flex gap-2 lg:gap-3">
                  <button
                    ...
                    className={`px-6 py-3 rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl ${isActive
                  ...
                  <button
                    ...
                    className={`px-6 py-3 rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl ${sessionData.attempts.length === 0
                  ...
                  {sessionData.attempts.slice(-5).reverse().map(attempt => (
                    <div key={attempt.id} className={`p-4 rounded-xl border ${darkMode ? 'border-gray-700 text-gray-300' : 'border-gray-200 text-gray-700'} bg-transparent`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className={`text-lg font-bold ${attempt.confidence >= 80 ? 'text-green-600 dark:text-green-400' :
